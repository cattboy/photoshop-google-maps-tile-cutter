...
function cutTiles(options, tickCallback) {
	...
	//Cattboy Edit Changed AnchorPosition.MIDDLECENTER to AnchorPosition.BOTTOMLEFT 
	curDoc.resizeCanvas(options.tileSize * Math.pow(2, maxZoomLevel), options.tileSize * Math.pow(2, maxZoomLevel), AnchorPosition.BOTTOMLEFT);
	...
	while (zoomLevel >= minZoomLevel) {
		...
		for (n = 1; n < numTilesTotal + 1; n++) {
			...
			if (options.saveTransparentTiles || !isLayerEmpty(curDoc, curDoc.activeLayer)) {
				...
				//Cattboy Edit... Commented this out so the 'leftover tiles' are transparent
